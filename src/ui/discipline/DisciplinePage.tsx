  ...
  if (!disciplineOrder.includes(name)) {
    return (
      <div className="px-4 py-16 text-center text-sm text-ink-2">
        ...
        <Link to={procurementModule.path} className="text-ai-1 underline">
  ...
  return (
    <Stagger className="mx-auto grid max-w-[1600px] gap-4 px-4 py-4">
      <motion.div layoutId={`discipline-${name}`} className="flex flex-wrap items-center gap-3">
        <Link to={{ pathname: procurementModule.path, search: query ? `?${query}` : '' }} className="text-sm text-ink-3 hover:text-ink">
          ← {t('discipline.backToOverview')}
        </Link>
        ...
        <nav className="ml-auto flex flex-wrap gap-1 text-xs" aria-label={t('discipline.otherDisciplines')}>
          {disciplineOrder
            .filter((d) => d !== name)
            .map((d) => (
              <Link key={d} to={{ pathname: `${procurementModule.path}/discipline/${encodeURIComponent(d)}`, search: query ? `?${query}` : '' }} className="rounded-full border border-line px-2 py-1 text-ink-3 hover:text-ink">
                {d}
              </Link>
            ))}
        </nav>
